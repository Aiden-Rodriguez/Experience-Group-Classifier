...
def parse_egg_groups(val) -> list[str]:
    """Convert tilde-separated egg groups to a list[str], normalized lowercase."""
...
def collapse_sidecar(df2: pd.DataFrame) -> pd.DataFrame:
    """
    Collapse pokemon_2 to one row per normalized name, merging egg_groups.
    Keeps 'name' so we can use the canonical child display name.
    Also carries over battle stats (Hp, attack, sp_attack, defense, sp_defense, speed).
    """
...
def build_evolves_to_map(right_collapsed: pd.DataFrame, allowed_norms: set[str]) -> dict[str, list[str]]:
    """
    Build mapping parent_norm -> [child names], but ONLY include children whose
    normalized names are present in allowed_norms (left table’s names).
    """
...
def run(in_path: Path, side_path: Path | None, out_path: Path):
    ...
    # Remove stat columns from REQUIRED since we're sourcing them from sidecar now.
    required = ["name","pokedex_number","generation","abilities","type1","type2",
                "weight_kg","is_legendary","base_total",  # base_total will be recomputed
    ...
    if side_path and Path(side_path).exists():
        ...
        def attach_fill_from_left(df: pd.DataFrame, col: str) -> pd.DataFrame:
            """Traditional behavior: prefer left value, fill from side if missing."""
        ...
        def attach_prefer_side(df: pd.DataFrame, col: str) -> pd.DataFrame:
            """New behavior for stats: prefer side value, fall back to left if side missing."""
            if col not in df.columns:
                df[col] = np.nan
            sidecol = f"{col}_side"
            if sidecol in df.columns:
    ...
    # Recompute base_total from stats
    base_total_series = (
        left["Hp"].fillna(0) +
